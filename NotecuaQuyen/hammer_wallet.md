...
| --- | --- | --- |
| Learner | Web/Mobile checkout, My Learning, lịch sử giao dịch | Phát sinh debit purchase, credit refund |
...
| --- | --- | --- | --- |
| Web/Mobile checkout class | Cart Checkout | Order, Order Item, Paid Item, Transfer | buyer debit + seller credit |
| Admin refund class | RefundsController#update | Refund, Transfer | buyer credit + seller debit |
...
| Web/Mobile checkout event | CheckoutEventOrder / PayNowEvent | EventUser, EventUserItem, EventUserTicket | buyer debit + event seller credit |
| Admin refund event | EventRefundsController#update | EventRefund | buyer credit + seller debit (mục tiêu), hiện chưa đủ |
...
| event_payout_request/event_payout_processed | Đang là hướng triển khai Wallet phase cho Event payout |
...
| --- | --- | --- | --- |
| class_purchase | commerce | debit buyer | buyer_available -, seller_class_pending + |
| event_purchase | commerce | debit buyer | buyer_available -, seller_event_pending + |
| class_refund | adjustment | credit buyer | buyer_available +, seller_class_pending/class_available - |
| event_refund | adjustment | credit buyer | buyer_available +, seller_event_pending/event_available - |
| event_payout_request | payout (event) | lock funds | event_available -> event_locked_for_payout |
| event_payout_processed | payout (event) | debit seller wallet | event_locked_for_payout - |
| event_payout_failed | payout (event) | unlock funds | event_locked_for_payout -, event_available + |
...
3. Credit/debit phải có cặp đối ứng buyer-seller khi là commerce/refund.
4. Pending chỉ chuyển sang Available qua rule release rõ ràng (scheduler/policy).
5. Wallet chỉ xử lý event payout: trích từ event_available; nếu fail phải hoàn lock về event_available.
6. Instructor/Class payout giữ nguyên flow legacy hiện tại (không migrate trong phase này).

### 16.4 Mermaid flow riêng cho Hammer Wallet

```mermaid
flowchart LR
  A[Class/Event Purchase] --> B[Ledger Entry: pending]
  B --> C[Release Job]
  C --> D[available]
  D --> E[Event Payout Request]
  E --> F[event_locked_for_payout]
  F --> G[event_payout_processed]
  B --> H[Refund Approved]
  H --> I[Refund Entry: buyer credit]
  I --> J[Seller deduction by stream]
  K[Instructor/Class Payout] --> L[Legacy flow outside Wallet scope]
```
