...
- Khi processed: giảm locked và ghi payout_processed.
...
- My Balance hiển thị 1 số tổng + breakdown:
	...
	- Pending
- Số tiền cho phép payout trong Wallet phase = `event_available`.
...
- Event tab:
	- Tiền vào: event_refund, event_earning.
	- Tiền ra: event_purchase, event_payout_request.
- Course tab:
	- Tiền vào: class_refund, class_earning.
...
- Bố cục theo bill/order detail:
	...
	- Payment method + provider (Stripe/Airwallex)
	- Thời gian phát sinh
	- Reference ID có nút copy
	- Thanh tiến độ trạng thái (pending -> processing -> completed/failed/refunded)
...
2. Buyer wallet ghi debit class_purchase.
3. Seller wallet ghi credit class_earning.
4. Tiền vào pending hoặc available theo policy hold.

### 8.2 Luồng mua event

1. Buyer thanh toán vé event.
2. Buyer wallet ghi debit event_purchase.
3. Event owner wallet ghi credit event_earning.
4. Tiền vào pending hoặc available theo policy event.

### 8.3 Luồng refund class/event

- Buyer nhận credit refund.
- Seller nhận debit deduction tương ứng stream.
- Trừ pending trước, thiếu mới trừ available.
